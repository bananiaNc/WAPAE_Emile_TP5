...
L'erreur affiché dans la console est errno: -4058.
...
<https://npmjs.org/package/loglevel> loglevel
<https://npmjs.org/package/morgan> morgan
<https://npmjs.org/package/http-errors> http-errors
...
Il faudra ajouter le contenu de script ci-dessous à l'intérieur du contenu script du package.json
```json
{
  "script": {
    "express-dev": "cross-env NODE_ENV=development nodemon server-exprexx.mjs",
    "express-prod": "cross-env NODE_ENV=production node server-http.mjs"
  }
}
```
...
L'évènement listening est déclenché lorsque le serveur commence à écouter sur le port (port) et l'hôte (host) spécifié.
**Commit/push** dans votre dépot Git.
